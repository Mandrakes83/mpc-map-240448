...
public_vars.path(1:25,:) = [linspace(2.5,10,25);linspace(2,8,25)]';
public_vars.path(26:50,:) = [linspace(10,16,25);linspace(8,6,25)]';
public_vars.path(51:75,:) = [linspace(16,16,25);linspace(6,2,25)]';
...
elseif(read_only_vars.counter == Init_cycle_count-1)
    ...
    public_vars.sigma(3,3) = (pi/8)^2;

    
    public_vars.kf.Q = diag([
            0.005^2;                 % x process noise
            0.02^2;                 % y process noise
            0.02^2            % theta process noise
        ]);
